  ...
  for(const endpoint of [...new Set(endpoints)]){
    try{
      ...
      try{
        ...
        if(/captcha/i.test(msg))throw new Error('سيرفر KF يطلب تحقق CAPTCHA من KrakenFiles حاليًا');
        const u=firstJsonUrl(json);if(u&&isAllowedFetchUrl(u)&&!providerPage(u))return cleanHtmlUrl(u);
      }catch(e:any){if(/CAPTCHA/i.test(String(e?.message||'')))throw e;}
...
async function resolveProvider(server:any,source:string){
  ...
  const opened=await fetchHtml(source);if(opened.direct)return opened.direct;return resolveFromHtml(server,opened.html);
}
...
export async function GET(req:NextRequest,{params}:{params:Promise<{id:string;episodeId:string}>}){
  ...
  try{
    if(final&&looksDirect(final)){}else{
      ...
      const embeddedDirect=candidates.find(looksDirect);if(embeddedDirect)final=embeddedDirect;else{const source=candidates.find(isAllowedFetchUrl);if(!source)throw new Error('لا يوجد رابط مصدر صالح لهذا السيرفر');final=await resolveProvider(server,source);}
    }
    if(!final||!isAllowedFetchUrl(final)||providerPage(final))throw new Error('لم نتمكن من استخراج رابط الملف المباشر');
    const constants=await getDocument('Settings/constants').catch(()=>null);const bunnyEnabled=Boolean(constants?.bu_auth_enabled);const bunnyKey=process.env.BUNNY_AUTH_KEY||String(constants?.bu_auth_key||'');
